...
func _input(event: InputEvent) -> void:
	if event.is_action_pressed("menu_cancel") or event.is_action_pressed("pause_game") or event.is_action_pressed("ui_accept"): #or event.is_action_pressed("menu_accept"):
		Loader.loadingScreen2Scene(toLevelFile)
